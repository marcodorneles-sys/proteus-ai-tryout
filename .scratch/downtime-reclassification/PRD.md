...
22. As a shop-floor operator, I want a correction to create a new Correction Record and the timeline to show only the latest, so that the current state is always unambiguous.
...
- **Reason Eligibility Filter** (deep, pure). Input: a selected span (or a set of
  ...
  catalog filter at selection time, never as a post-hoc validation error.
...
- **Reclassification Service** (thin, side-effecting). Orchestrates the four pure
  modules to handle reclassify, bulk reclassify, correct, and void commands. It
  validates against the Writable Window Policy and Reason Eligibility Filter,
  then creates `Downtimes`, appends `Correction Records`, and performs voids. It
  never mutates a `Machine Stop`.
...
- Edits are append-only as `Correction Records`; rendering is as-now (latest
...
- Audit / as-of-T historical reconstruction (a separate feature); this PRD only
  guarantees as-now rendering and the retention of `Correction Records`.
...
- `CONTEXT.md` (root glossary) was created during the session and is the source
  of truth for the terms used throughout this PRD.
